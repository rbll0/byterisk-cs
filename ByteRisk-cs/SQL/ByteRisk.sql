...
-- Inserindo registros na tabela Beneficiário
...
-- Inserindo registros na tabela Profissional
...
INSERT INTO Consulta (id_beneficiario, data_consulta, motivo_consulta, status, id_profissional)
VALUES (10, TO_DATE('2023-09-25', 'YYYY-MM-DD'), 'Consulta de emergência', 'Agendada', 5);
...
-- Inserindo registros na tabela Sinistro
...
-- Habilita saída de dados dbms
set serveroutput on
set verify off

-- INNER JOIN
BEGIN
    -- Consulta para exibir Beneficiários e seus respectivos Planos
FOR reg IN (
        SELECT b.nome AS beneficiario, p.nome_plano AS plano
        FROM Beneficiario b
        INNER JOIN Plano p ON b.id_plano = p.id_plano
    ) LOOP
        DBMS_OUTPUT.PUT_LINE('Beneficiário: ' || reg.beneficiario || ' | Plano: ' || reg.plano);
END LOOP;
END;

-- LEFT JOIN
BEGIN
    -- Consulta para exibir Consultas e seus Profissionais (ou NULL se não houver)
FOR reg IN (
        SELECT c.id_consulta, p.nome AS profissional, c.data_consulta
        FROM Consulta c
        LEFT JOIN Profissional p ON c.id_profissional = p.id_profissional
    ) LOOP
        DBMS_OUTPUT.PUT_LINE('Consulta ID: ' || reg.id_consulta || ' | Profissional: ' || NVL(reg.profissional, 'Sem profissional') || ' | Data: ' || reg.data_consulta);
END LOOP;
END;

-- RIGHT JOIN
BEGIN
    -- Consulta para exibir Profissionais e suas Consultas (ou NULL se não houver)
FOR reg IN (
        SELECT p.nome AS profissional, c.id_consulta, c.data_consulta
        FROM Profissional p
        RIGHT JOIN Consulta c ON p.id_profissional = c.id_profissional
    ) LOOP
        DBMS_OUTPUT.PUT_LINE('Profissional: ' || NVL(reg.profissional, 'Sem profissional') || ' | Consulta ID: ' || reg.id_consulta || ' | Data: ' || reg.data_consulta);
END LOOP;
END;

-- GROUP BY E ORDER BY
BEGIN
    -- Consulta para exibir a quantidade de beneficiários por plano e ordenar pelo valor do plano
FOR reg IN (
        SELECT p.nome_plano, COUNT(b.id_beneficiario) AS total_beneficiarios, p.valor_mensal
        FROM Plano p
        LEFT JOIN Beneficiario b ON p.id_plano = b.id_plano
        GROUP BY p.nome_plano, p.valor_mensal
        ORDER BY p.valor_mensal DESC
    ) LOOP
        DBMS_OUTPUT.PUT_LINE('Plano: ' || reg.nome_plano || ' | Beneficiários: ' || reg.total_beneficiarios || ' | Valor Mensal: ' || reg.valor_mensal);
END LOOP;
END;

-- Bloco para dar update no valor do plano
DECLARE
v_id_plano NUMBER := 1; -- Variável para o ID do plano
    v_novo_valor DECIMAL(10, 2) := 299.99; -- Variável para o novo valor mensal
    v_count NUMBER;
BEGIN
...
IF v_count > 0 THEN
        -- Realiza o UPDATE se o plano existir
UPDATE Plano
SET valor_mensal = v_novo_valor
WHERE id_plano = v_id_plano;

DBMS_OUTPUT.PUT_LINE('Plano com ID ' || v_id_plano || ' atualizado com sucesso. Novo valor: ' || v_novo_valor);
ELSE
        DBMS_OUTPUT.PUT_LINE('Plano com ID ' || v_id_plano || ' não encontrado.');
END IF;
END;

-- Bloco para deletar uma consulta
DECLARE
v_id_consulta NUMBER := 1; -- Variável para o ID da consulta
    v_count NUMBER;
BEGIN
...
IF v_count > 0 THEN
        -- Realiza o DELETE se a consulta existir
DELETE FROM Consulta
WHERE id_consulta = v_id_consulta;

DBMS_OUTPUT.PUT_LINE('Consulta com ID ' || v_id_consulta || ' deletada com sucesso.');
ELSE
        DBMS_OUTPUT.PUT_LINE('Consulta com ID ' || v_id_consulta || ' não encontrada.');
END IF;
END;
